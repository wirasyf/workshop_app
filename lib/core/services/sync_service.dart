import 'dart:async';
import 'dart:convert';

import 'package:flutter_riverpod/flutter_riverpod.dart';
import '../database/app_database.dart';
import 'connectivity_service.dart';
import 'supabase_service.dart';

/// Provider untuk sync service
final syncServiceProvider = Provider<SyncService>((ref) {
  final db = ref.watch(databaseProvider);
  return SyncService(db);
});

/// Provider untuk database
final databaseProvider = Provider<AppDatabase>((ref) {
  throw UnimplementedError('databaseProvider harus di-override di ProviderScope');
});

/// Service sinkronisasi offline-first
/// Mengirim perubahan lokal ke Supabase saat online
class SyncService {
  final AppDatabase _db;
  Timer? _syncTimer;

  SyncService(this._db);

  /// Mulai periodic sync setiap 30 detik
  void startPeriodicSync() {
    _syncTimer?.cancel();
    _syncTimer = Timer.periodic(const Duration(seconds: 30), (_) => syncPendingChanges());
  }

  void stopPeriodicSync() {
    _syncTimer?.cancel();
    _syncTimer = null;
  }

  /// Tambah entry ke antrian sync
  Future<void> enqueue({
    required String tableName,
    required int recordId,
    required String operation,
    required Map<String, dynamic> data,
  }) async {
    await _db.addToSyncQueue(SyncQueueCompanion.insert(
      syncTableName: tableName,
      recordId: recordId,
      operation: operation,
      data: jsonEncode(data),
    ));
  }

  /// Proses semua pending sync items
  Future<void> syncPendingChanges() async {
    final isOnline = await ConnectivityService.isOnline();
    if (!isOnline) return;

    final pendingItems = await _db.getPendingSyncItems();
    if (pendingItems.isEmpty) return;

    final client = SupabaseService.client;

    for (final item in pendingItems) {
      try {
        final data = jsonDecode(item.data) as Map<String, dynamic>;

        switch (item.operation) {
          case 'create':
            await client.from(item.syncTableName).insert(data);
            break;
          case 'update':
            await client.from(item.syncTableName).update(data).eq('id', item.recordId);
            break;
          case 'delete':
            await client.from(item.syncTableName).delete().eq('id', item.recordId);
            break;
        }

        await _db.markSynced(item.id);
      } catch (e) {
        // Gagal sync, akan dicoba lagi di cycle berikutnya
        // ignore dan lanjut ke item berikutnya
      }
    }
  }
}
