...
class _ServiceFormScreenState extends ConsumerState<ServiceFormScreen> {
  ...
  Future<void> _loadService() async {
    try {
      ...
      final service = await repo.getServiceById(widget.serviceId!).first;
  ...
  Future<void> _saveService() async {
    ...
    try {
      final id = isEditing ? widget.serviceId! : const Uuid().v4();
  ...
  Future<void> _deleteService() async {
    ...
    final repo = ref.read(serviceRepositoryProvider);
    await repo.deleteService(widget.serviceId!);
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            if (isEditing) IconButton(icon: const Icon(Icons.delete_rounded, color: AppColors.error), onPressed: _deleteService),
          ],
        ),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Nama Jasa *', hintText: 'Contoh: Ganti Oli, Servis Rutin', prefixIcon: Icon(Icons.build_rounded)),
              validator: (v) => (v == null || v.trim().isEmpty) ? 'Nama jasa wajib diisi' : null,
            ),
            const SizedBox(height: 16),
            TextFormField(
              controller: _descCtrl,
              decoration: const InputDecoration(labelText: 'Deskripsi', hintText: 'Detail layanan (opsional)', prefixIcon: Icon(Icons.description_rounded)),
              maxLines: 3,
            ),
            const SizedBox(height: 16),
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Harga *', prefixText: 'Rp ', prefixIcon: Icon(Icons.payments_rounded)),
              validator: (v) {
            ...
            TextFormField(
              ...
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Estimasi Waktu (menit)', suffixText: 'menit', prefixIcon: Icon(Icons.timer_rounded)),
              validator: (v) {
            ...
            Consumer(
              builder: (context, ref, _) {
                ...
                return catsAsync.when(
                  ...
                  data: (cats) {
                    if (cats.isEmpty) {
                      return OutlinedButton.icon(onPressed: () => context.push('/services/categories'), icon: const Icon(Icons.add_rounded), label: const Text('Buat Kategori Jasa'));
                    }
                    return DropdownButtonFormField<String>(
                      ...
                      decoration: InputDecoration(
                        ...
                        suffixIcon: IconButton(icon: const Icon(Icons.settings_rounded, size: 20), onPressed: () => context.push('/services/categories'), tooltip: 'Kelola Kategori'),
                      ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isLoading ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(isEditing ? 'Simpan Perubahan' : 'Simpan Jasa'),
              ),
            ),
          ],
        ),
