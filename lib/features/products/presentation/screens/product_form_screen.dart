...
class _ProductFormScreenState extends ConsumerState<ProductFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        AppToast.show(context, _isEditing ? 'Produk diperbarui' : 'Produk ditambahkan', type: ToastType.success);
          ...
          children: [
            Center(
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.border)),
                  child: _imageFile != null
                      ...
                      ? ClipRRect(
                          ...
                          child: _imagePath!.startsWith('http') 
                            ? CachedNetworkImage(imageUrl: _imagePath!, fit: BoxFit.cover, placeholder: (_, __) => const Center(child: CircularProgressIndicator()), errorWidget: (_, __, ___) => const Icon(Icons.error_rounded))
                            : Image.file(File(_imagePath!), fit: BoxFit.cover),
                        )
                      : const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_a_photo_rounded, color: AppColors.primary, size: 32), SizedBox(height: 4), Text('Tambah Foto', style: TextStyle(fontSize: 11, color: AppColors.primary))]),
                ),
              ),
            ),
            const SizedBox(height: 24),
            _sectionTitle('Informasi Dasar'),
            _field(_nameCtrl, 'Nama Produk *', validator: (v) => v!.isEmpty ? 'Wajib diisi' : null),
            Row(
              children: [
                ...
                Expanded(
                  child: _field(
                    _barcodeCtrl, 'Barcode',
                    suffix: IconButton(
                      icon: const Icon(Icons.qr_code_scanner_rounded, size: 20),
                      onPressed: () async {
                        final code = await Navigator.push<String>(context, MaterialPageRoute(builder: (_) => const BarcodeScannerDialog()));
            ...
            Row(
              children: [
                Expanded(
                  child: _field(
                    _costPriceCtrl, 'Harga Beli *', keyboard: TextInputType.number, prefix: 'Rp ', formatters: [RupiahInputFormatter()],
                    validator: (v) {
                ...
                Expanded(
                  child: _field(
                    _sellPriceCtrl, 'Harga Jual *', keyboard: TextInputType.number, prefix: 'Rp ', formatters: [RupiahInputFormatter()],
                    validator: (v) {
                      if (v == null || v.isEmpty) return 'Wajib';
                      if (CurrencyFormatter.parse(v) <= 0) return 'Harus > 0';
                      return null;
            ...
            Row(
              children: [
                Expanded(
                  child: _field(
                    _stockQtyCtrl, 'Stok Awal', keyboard: TextInputType.number,
                    validator: (v) {
                      if (v == null || v.isEmpty) return 'Wajib';
                ...
                Expanded(
                  child: _field(
                    _stockMinCtrl, 'Stok Minimum', keyboard: TextInputType.number,
                    validator: (v) {
                      if (v == null || v.isEmpty) return 'Wajib';
                      if ((int.tryParse(v) ?? -1) < 0) return 'Harus >= 0';
                      return null;
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isLoading ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(_isEditing ? 'Simpan Perubahan' : 'Tambah Produk'),
              ),
            ),
          ],
        ),
  ...
  Widget _sectionTitle(String title) => Padding(padding: const EdgeInsets.only(bottom: 12, top: 4), child: Text(title, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)));

  Widget _field(TextEditingController ctrl, String label, {TextInputType keyboard = TextInputType.text, String? Function(String?)? validator, Widget? suffix, String? prefix, List<TextInputFormatter>? formatters}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: TextFormField(controller: ctrl, keyboardType: keyboard, validator: validator, inputFormatters: formatters, decoration: InputDecoration(labelText: label, suffixIcon: suffix, prefixText: prefix)),
    );
  }
}
