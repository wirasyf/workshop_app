...
class ReportScreen extends ConsumerWidget {
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(children: ReportPeriod.values.map((p) {
              final label = switch (p) { ReportPeriod.daily => 'Harian', ReportPeriod.weekly => 'Mingguan', ReportPeriod.monthly => 'Bulanan' };
              return Expanded(child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 4),
                  child: ChoiceChip(
                    label: Text(label),
                    selected: period == p,
                    onSelected: (_) => ref.read(reportPeriodProvider.notifier).state = p,
                  ...
                  children: [
                    Row(children: [
                      Expanded(child: MetricCard(
                        label: 'Omzet', value: CurrencyFormatter.formatCompact(data['totalSales'] ?? 0),
                    ...
                    // Produk terlaris
                    Text('Produk Terlaris', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    if (top.isEmpty)
                      const Padding(padding: EdgeInsets.all(16), child: Text('Belum ada data'))
                    else
                      ...top.asMap().entries.map((e) {
                        final p = e.value;
                        final txnCount = p['totalQty'] ?? 0;
                        return Container(
                          ...
                          child: Row(children: [
                            CircleAvatar(
                              radius: 18,
                              backgroundColor: AppColors.chartColors[e.key % AppColors.chartColors.length].withValues(alpha: 0.15),
                              child: Text('${e.key + 1}', style: TextStyle(color: AppColors.chartColors[e.key % AppColors.chartColors.length], fontWeight: FontWeight.w700, fontSize: 14)),
                            ),
                            const SizedBox(width: 12),
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text(p['name']?.toString() ?? 'Produk', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                              const SizedBox(height: 2),
                              Text('$txnCount transaksi', style: theme.textTheme.bodySmall?.copyWith(color: AppColors.textSecondary)),
                            ])),
                            Text(CurrencyFormatter.formatCompact((p['totalRevenue'] as num?)?.toDouble() ?? 0),
                      ...
                      ...txns.take(10).map((txn) => Container(
                        ...
                        child: Row(children: [
                          Container(
                            ...
                            child: const Icon(Icons.receipt, color: AppColors.success, size: 18),
                          ),
                          const SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(txn.invoiceNo, style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
                            const SizedBox(height: 2),
                            Text(DateFormatter.formatWithTime(txn.createdAt), style: theme.textTheme.bodySmall?.copyWith(color: AppColors.textSecondary, fontSize: 11)),
                          ])),
                          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                            Text(CurrencyFormatter.format(txn.total), style: theme.textTheme.titleSmall?.copyWith(color: AppColors.primary, fontWeight: FontWeight.w700)),
                            const SizedBox(height: 2),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                              decoration: BoxDecoration(color: AppColors.successLight, borderRadius: BorderRadius.circular(20)),
                              child: Text(txn.paymentMethod.toUpperCase(), style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: AppColors.success)),
                            ),
                          ]),
                        ]),
                      )),
  ...
  Widget _buildChart(BuildContext context, List<Map<String, dynamic>> daily, ReportPeriod period) {
    ...
    final now = DateTime.now();
    final displaySales = List.generate(chartDays, (index) {
      final d = now.subtract(Duration(days: chartDays - 1 - index));
      final dateStr = '${d.year}-${d.month.toString().padLeft(2, '0')}-${d.day.toString().padLeft(2, '0')}';
      final hasData = daily.any((e) => (e['date'] as String).startsWith(dateStr));
      final total = hasData
          ? daily.firstWhere((e) => (e['date'] as String).startsWith(dateStr))['total']
          : 0.0;
        ...
        barGroups: displaySales.asMap().entries.map((e) {
          return BarChartGroupData(x: e.key, barRods: [
            BarChartRodData(
              ...
              borderRadius: BorderRadius.vertical(top: Radius.circular(isMonthly ? 2 : 6)),
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (idx >= 0 && idx < displaySales.length) {
                ...
                return SideTitleWidget(
                  ...
                  child: Text(label, style: TextStyle(fontSize: isMonthly ? 8 : 9, color: AppColors.textPrimary, fontWeight: FontWeight.w600)),
                );
              }
              return SideTitleWidget(meta: meta, child: const SizedBox());
            },
  ...
  Future<void> _exportReport(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      final now = DateTime.now();
      final fileName = 'laporan_${periodLabel.toLowerCase()}_${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}.csv';
